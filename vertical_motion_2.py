...
def create_vertical_motion_robot(name, pos, rotation):
    ...
    ET.SubElement(guide, "geom", name=f"{name}_guide_geom", type="cylinder", size="0.025 0.9", pos="0 0 0.4")
...
for i in range(1, 5):
    ...
    world.actuator.append(new_actuator(joint=f"robot{i}_vertical_joint2", act_type="position", name=f"{i}_actuator_joint2", kp="10000", kv='600'))
...
camera = ET.SubElement(world.worldbody, "camera", name="overview", pos="0 0 3", mode="fixed", quat="0.3827 0 0 0.9239", fovy="45")
...
if __name__ == "__main__":
    ...
    # Load and modify the XML to remove the unrecognized attributes
    ...
    tree.write(model_xml_path)

    # Load the model from the modified XML file using mujoco_py
    ...
    viewer.cam.type = 0
    viewer.cam.fixedcamid = sim.model.camera_name2id("overview")
